...
(define (transpose-element x amount) ;Virker ikke helt endnu makkeres
  (if(null? x)
     '()
     (if(procedure? x)
        (cond((eqv? 'Note (send 'get-type x)) (send 'transpose amount x))
             ((eqv? 'Pause (send 'get-type x)) x)
             ((eqv? 'SequentialMusicElement (send 'get-type x)) (send 'transpose amount x)) ;(SequentialMusicElement (car (transpose-element (send 'get-elements x) amount))))
             ((eqv? 'ParallelMusicElement (send 'get-type x)) (ParallelMusicElement(car (transpose-element (send 'get-elements x) amount ))))
             (else error "Error: Unkown type"))
...
;For testing
(define tnote (note 90 1500 5 0))
(define tnote2 (note 80 1500 4 0))
